...
└──────────────────────────────────────────────────────┘
        ↓
  [ FastAPI :8000 ]             ← Step 3: REST API (16 endpoints)
        ↓
  [ Dash Dashboard :8050 ]      ← Step 4: 7-page Interactive Dashboard
  ...
  └── Neo4j Graph Queries       ← Shortest path algorithm
        ↓
  [ Docker Compose ]            ← Step 5: 7 containers, one command
```

**Key Architectural Decision:**  
Dashboard communicates **exclusively through FastAPI endpoints** — no direct database access.  
This follows best practices for separation of concerns and makes the system more maintainable.
...
│   ├── data.py              ← Data layer (API client → mock fallback)
...
- **Features:** Airline (encoded), Origin (encoded), Destination (encoded), Distance
...
```
P(delay=1|X) = 1 / (1 + exp(-(β₀ + β₁X₁ + β₂X₂ + β₃X₃ + β₄X₄)))
```

Where:
- X₁ = Airline (LabelEncoded: American Airlines → 0, Delta → 1, etc.)
- X₂ = Origin airport (LabelEncoded: 0-345)
- X₃ = Destination airport (LabelEncoded: 0-345)
- X₄ = Distance (numeric)
- β values = model weights learned during training

The model's `predict_proba()` method returns the probability directly.

**Risk Levels:**
- < 30% probability = **LOW RISK** (green)
- 30-60% probability = **MEDIUM RISK** (yellow)
- > 60% probability = **HIGH RISK** (red)
...
**Added for separation of concerns — Dashboard now uses API instead of direct database access**

| Method | Endpoint | Database | Description |
|--------|----------|----------|-------------|
| GET | `/api/flights` | PostgreSQL | Get flights for dashboard (random sampling) |
| GET | `/api/airlines` | PostgreSQL | List all airlines |
| GET | `/api/origins` | PostgreSQL | List origin airports (optional airline filter) |
| GET | `/api/destinations` | PostgreSQL | List destinations (airline/origin filter) |
| GET | `/api/dashboard-stats` | PostgreSQL | Aggregated stats (total flights, delay rate, etc.) |
...
**Total: 16 endpoints (11 original + 5 new dashboard endpoints)**
...
| Member | Role |
|--------|------|
| **Ali Doghan** | Dashboard (7 pages, OOP), ML models, Neo4j graph, live weather API, API endpoints refactoring, Docker setup, documentation |
